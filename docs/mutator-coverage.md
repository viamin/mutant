...
| Bang reductions | `covered` | `"array.map!(&:to_s)" -> array.map(&:to_s)` | A dedicated bang-to-non-bang reduction emits the trailing-`!`-stripped selector for any non-operator send such as `map!`, `compact!`, `sort!`, or `uniq!`; operator selectors like `:!` and `:!=` are skipped. |
