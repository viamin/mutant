  ...
  describe '.call' do
    ...
    let(:raw_expectations) do
      [
        ...
        {
          receiver:  Mutant::Results::Writer,
          selector:  :call,
          arguments: [env_result]
        }
      ]
    ...
    context 'when writing results fails' do
      let(:writer_error_class) do
        Class.new(RuntimeError) do
          ...
          def to_s
            'disk full with backtrace context'
          end
        end
      end
      let(:writer_error) { writer_error_class.new('disk full') }
      let(:raw_expectations) do
        [
          {
            receiver:  reporter,
          ...
          },
          {
            ...
            selector:  :call,
            arguments: [env_result],
            reaction:  { exception: writer_error }
          },
          {
            receiver:  reporter,
            selector:  :warn,
            arguments: ['Results writer failed: disk full']
          }
        ]
      end

      it 'warns and still returns the mutation result' do
        verify_events { expect(apply).to eql(env_result) }
      end
    end
  end
