...
Every `mutant run` invocation writes a YAML summary to `.mutant/results/<timestamp>-<short-sha>-<run-id>.yml`.
Use `--results-dir DIRECTORY` to override that location. Files accumulate by design.

The document uses string keys and is compatible with:
